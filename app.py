...
def build_chart(df, ticker_name, support_levels=None, resistance_levels=None, show_bb=True, show_macd=True, show_fib=False):
close = df[‘Close’]
high = df[‘High’]
low = df[‘Low’]
volume = df[‘Volume’]
...
if resistance_levels:
    for r in sorted(resistance_levels)[:4]:
        if r > 0:
            fig.add_hline(y=r, line=dict(color="rgba(255,59,59,0.55)", width=1.2, dash="dot"),
                          annotation_text=f"R {r:.2f}", annotation_position="right",
                          annotation_font=dict(color="#FF6B6B", size=10), row=1, col=1)
if support_levels:
    for s in sorted(support_levels, reverse=True)[:4]:
        if s > 0:
            fig.add_hline(y=s, line=dict(color="rgba(0,255,136,0.55)", width=1.2, dash="dot"),
                          annotation_text=f"S {s:.2f}", annotation_position="right",
                          annotation_font=dict(color="#00FF88", size=10), row=1, col=1)

# ── Fibonacci Retracement
if show_fib:
    fib_high = df["High"].tail(60).max()
    fib_low  = df["Low"].tail(60).min()
    fib_levels = calc_fibonacci(fib_high, fib_low)
    fib_colors = {"23.6%": "#FFD700", "38.2%": "#FFA500",
                  "50.0%": "#FF8C00", "61.8%": "#FF6347", "78.6%": "#FF4500"}
    for label, val in fib_levels.items():
        if label in fib_colors:
            fig.add_hline(y=val,
                          line=dict(color=fib_colors[label], width=1, dash="dashdot"),
                          annotation_text=f"Fib {label} {val:.2f}",
                          annotation_position="left",
                          annotation_font=dict(color=fib_colors[label], size=9),
                          row=1, col=1)

# ── EMA Crossover annotations
ema20_arr = ema20.values
ema50_arr = ema50.values
for i in range(1, min(len(ema20_arr), len(ema50_arr))):
    prev_diff = ema20_arr[i-1] - ema50_arr[i-1]
    curr_diff = ema20_arr[i]   - ema50_arr[i]
    if pd.isna(prev_diff) or pd.isna(curr_diff):
        continue
    if prev_diff < 0 and curr_diff > 0:
        fig.add_annotation(x=df.index[i], y=ema20_arr[i],
                           text="Golden Cross", showarrow=True,
                           arrowhead=2, arrowcolor="#FFD700",
                           font=dict(color="#FFD700", size=10),
                           bgcolor="rgba(10,15,30,0.85)",
                           bordercolor="#FFD700", row=1, col=1)
    elif prev_diff > 0 and curr_diff < 0:
        fig.add_annotation(x=df.index[i], y=ema20_arr[i],
                           text="Death Cross", showarrow=True,
                           arrowhead=2, arrowcolor="#FF3B3B",
                           font=dict(color="#FF3B3B", size=10),
                           bgcolor="rgba(10,15,30,0.85)",
                           bordercolor="#FF3B3B", row=1, col=1)
...
st.session_state["show_fib"] = st.toggle("Fibonacci Retracement", value=False)
...
if df is None or len(df) < 50:
    ...
    with st.spinner("🎨 بناء الشارت..."):
        show_fib = st.session_state.get("show_fib", False)
    fig, rsi, macd_line, signal_line, histogram, ema20, ema50, atr, k_stoch, d_stoch, bb_upper, bb_lower = build_chart(df, f"{ticker} — {asset_type}", support_levels, resistance_levels, show_bb, show_macd, show_fib)
    ...
    # Quick Decision Card + Confidence + S/R Tables
    rsi_tmp = calc_rsi(close)
    rsi_now = rsi_tmp.iloc[-1]
    macd_tmp, sig_tmp, hist_tmp = calc_macd(close)
    ema20_tmp = calc_ema(close, 20).iloc[-1]
    ema50_tmp = calc_ema(close, 50).iloc[-1]
    vol_avg_tmp = df['Volume'].tail(20).mean()
    vol_ratio_now = df['Volume'].iloc[-1] / vol_avg_tmp if vol_avg_tmp > 0 else 1
    hist_rising = hist_tmp.iloc[-1] > hist_tmp.iloc[-2] if len(hist_tmp) > 1 else False
    bullish_signals = sum([ema20_tmp > ema50_tmp, macd_tmp.iloc[-1] > sig_tmp.iloc[-1], 40 < rsi_now < 70, hist_rising, vol_ratio_now > 1.0])
    confidence = int((bullish_signals / 5) * 100)
    conf_color = '#00FF88' if confidence >= 60 else '#FF3B3B' if confidence <= 40 else '#FFB800'
    conf_label = 'شراء قوي' if confidence >= 70 else 'بيع قوي' if confidence <= 30 else 'محايد' if confidence <= 55 else 'ميل شراء'
    atr_tmp = calc_atr(df['High'], df['Low'], close).iloc[-1]
    sl_p = round(price - 1.5 * atr_tmp, 4)
    t1_p = round(price + 1.5 * atr_tmp, 4)
    t2_p = round(price + 3.0 * atr_tmp, 4)
    rr_r = round((t1_p - price) / (price - sl_p), 2) if price != sl_p else 0
    st.markdown('<div class="section-header"><div class="section-title">⚡ بطاقة القرار السريع</div></div>', unsafe_allow_html=True)
    qc1, qc2, qc3, qc4, qc5 = st.columns(5)
    _cards = [('نقطة الدخول', f'{price:.4f}', '#D4AF37'), ('الهدف 1 🎯', f'{t1_p:.4f}', '#00FF88'), ('الهدف 2 🎯🎯', f'{t2_p:.4f}', '#00CC66'), ('وقف الخسارة 🛡️', f'{sl_p:.4f}', '#FF3B3B'), ('نسبة R:R', f'1:{rr_r}', '#00BFFF')]
    for _col, (_lbl, _val, _clr) in zip([qc1,qc2,qc3,qc4,qc5], _cards):
        with _col:
            st.markdown(f'<div class="metric-card"><div class="metric-label">{_lbl}</div><div class="metric-value" style="color:{_clr};font-size:1.05rem;">{_val}</div></div>', unsafe_allow_html=True)
    st.markdown('<div class="golden-divider"></div>', unsafe_allow_html=True)
    ...
    with col_r:
        res_list = sorted([r for r in resistance_levels if r > price])[:4] + sorted([r for r in resistance_levels if r <= price], reverse=True)[:1]
        rows_html = ''
        for r in sorted(res_list, reverse=True):
            dist = (r - price) / price * 100
            st_ = 'قريب 🔴' if abs(dist) < 3 else 'متوسط 🟠' if abs(dist) < 7 else 'بعيد ⚪'
            rows_html += f'<tr style="background:rgba(255,59,59,0.06)"><td style="color:#FF6B6B;font-weight:700;font-family:monospace">{r:.4f}</td><td style="color:#8899AA">{dist:+.2f}%</td><td style="color:#8899AA;font-size:0.82rem">{st_}</td></tr>'
        st.markdown(f'<table class="levels-table"><thead><tr><th>🔴 المقاومة</th><th>البُعد</th><th>القوة</th></tr></thead><tbody>{rows_html}</tbody></table>', unsafe_allow_html=True)
    with col_conf:
        bar_html = f'<div style="height:5px;width:{confidence}%;background:{conf_color};border-radius:3px"></div>'
        card_html = f'<div class="metric-card" style="text-align:center;padding:16px 8px;margin-top:4px"><div class="metric-label" style="font-size:0.7rem">CONFIDENCE</div><div style="font-family:Orbitron,monospace;font-size:1.8rem;font-weight:900;color:{conf_color}">{confidence}%</div><div style="font-size:0.75rem;color:{conf_color};margin-top:4px">{conf_label}</div><div style="margin-top:10px;height:5px;background:rgba(255,255,255,0.08);border-radius:3px">{bar_html}</div></div>'
        st.markdown(card_html, unsafe_allow_html=True)
    with col_s:
        sup_list = sorted([s for s in support_levels if s < price], reverse=True)[:4] + [s for s in support_levels if s >= price][:1]
        rows_html = ''
        for s in sorted(sup_list, reverse=True):
            dist = (s - price) / price * 100
            st_ = 'قريب 🟢' if abs(dist) < 3 else 'متوسط 🟡' if abs(dist) < 7 else 'بعيد ⚪'
            rows_html += f'<tr style="background:rgba(0,255,136,0.06)"><td style="color:#00FF88;font-weight:700;font-family:monospace">{s:.4f}</td><td style="color:#8899AA">{dist:+.2f}%</td><td style="color:#8899AA;font-size:0.82rem">{st_}</td></tr>'
        st.markdown(f'<table class="levels-table"><thead><tr><th>🟢 الدعم</th><th>البُعد</th><th>القوة</th></tr></thead><tbody>{rows_html}</tbody></table>', unsafe_allow_html=True)
    st.markdown('<div class="golden-divider"></div>', unsafe_allow_html=True)
