...
# TRADINGVIEW — رموز الأصول

# ══════════════════════════════════════════════════

# رموز TradingView: (symbol, exchange)

TV_SYMBOLS = {
# أسهم سعودية — Tadawul
“2222.SR”: (“2222”,  “TADAWUL”),
“2010.SR”: (“2010”,  “TADAWUL”),
“2030.SR”: (“2030”,  “TADAWUL”),
“2370.SR”: (“2370”,  “TADAWUL”),
“1120.SR”: (“1120”,  “TADAWUL”),
“1180.SR”: (“1180”,  “TADAWUL”),
“1050.SR”: (“1050”,  “TADAWUL”),
“1010.SR”: (“1010”,  “TADAWUL”),
“1060.SR”: (“1060”,  “TADAWUL”),
“1040.SR”: (“1040”,  “TADAWUL”),
“1080.SR”: (“1080”,  “TADAWUL”),
“1090.SR”: (“1090”,  “TADAWUL”),
“7010.SR”: (“7010”,  “TADAWUL”),
“7030.SR”: (“7030”,  “TADAWUL”),
“4002.SR”: (“4002”,  “TADAWUL”),
“1211.SR”: (“1211”,  “TADAWUL”),
“2280.SR”: (“2280”,  “TADAWUL”),
“2110.SR”: (“2110”,  “TADAWUL”),
“4082.SR”: (“4082”,  “TADAWUL”),
“5110.SR”: (“5110”,  “TADAWUL”),
“4012.SR”: (“4012”,  “TADAWUL”),
“4190.SR”: (“4190”,  “TADAWUL”),
“4160.SR”: (“4160”,  “TADAWUL”),
“2070.SR”: (“2070”,  “TADAWUL”),
“6163.SR”: (“6163”,  “TADAWUL”),
# عملات رقمية — Binance
...
“ATOM-USD”: (“ATOMUSDT”, “BINANCE”),
“NEAR-USD”: (“NEARUSDT”, “BINANCE”),
“FTM-USD”:  (“FTMUSDT”,  “BINANCE”),
“ALGO-USD”: (“ALGOUSDT”, “BINANCE”),
“XTZ-USD”:  (“XTZUSDT”,  “BINANCE”),
“HBAR-USD”: (“HBARUSDT”, “BINANCE”),
“UNI-USD”:  (“UNIUSDT”,  “BINANCE”),
“LINK-USD”: (“LINKUSDT”, “BINANCE”),
“AAVE-USD”: (“AAVEUSDT”, “BINANCE”),
“CRV-USD”:  (“CRVUSDT”,  “BINANCE”),
“MATIC-USD”:(“MATICUSDT”,“BINANCE”),
“OP-USD”:   (“OPUSDT”,   “BINANCE”),
“ARB-USD”:  (“ARBUSDT”,  “BINANCE”),
“DOGE-USD”: (“DOGEUSDT”, “BINANCE”),
“SHIB-USD”: (“SHIBUSDT”, “BINANCE”),
“PEPE-USD”: (“PEPEUSDT”, “BINANCE”),
“XRP-USD”:  (“XRPUSDT”,  “BINANCE”),
“XLM-USD”:  (“XLMUSDT”,  “BINANCE”),
“LTC-USD”:  (“LTCUSDT”,  “BINANCE”),
“FIL-USD”:  (“FILUSDT”,  “BINANCE”),
“FET-USD”:  (“FETUSDT”,  “BINANCE”),
“RNDR-USD”: (“RNDRUSDT”, “BINANCE”),
“AXS-USD”:  (“AXSUSDT”,  “BINANCE”),
“SAND-USD”: (“SANDUSDT”, “BINANCE”),
“MANA-USD”: (“MANAUSDT”, “BINANCE”),
“CRO-USD”:  (“CROUSDT”,  “BINANCE”),
# سلع — OANDA / TVC
“GC=F”:  (“GOLD”,   “OANDA”),
“CL=F”:  (“USOIL”,  “OANDA”),
“SI=F”:  (“SILVER”, “OANDA”),
“HG=F”:  (“COPPER”, “OANDA”),
# فوركس — OANDA
“EURUSD=X”: (“EURUSD”, “OANDA”),
“GBPUSD=X”: (“GBPUSD”, “OANDA”),
“USDJPY=X”: (“USDJPY”, “OANDA”),
“USDSAR=X”: (“USDSAR”, “OANDA”),
# أسهم عالمية — NASDAQ / NYSE
“^GSPC”: (“SPX”,  “SP”),
“^IXIC”: (“COMP”, “NASDAQ”),
“NVDA”:  (“NVDA”, “NASDAQ”),
“AAPL”:  (“AAPL”, “NASDAQ”),
...
# تحويل الإطار الزمني → tvDatafeed Interval
...
TV_BARS = {
“15m”: 500, “1h”: 500, “4h”: 400, “1d”: 350, “1wk”: 200,
}

# ── singleton للـ TvDatafeed (بدون تسجيل دخول = بيانات مجانية) ──
...
return None
try:
return TvDatafeed()   # بدون username/password = anonymous
except Exception:
return None
...
“”“سعر لحظي من Binance WebSocket REST — للكريبتو فقط”””
try:
...
“”“سعر لحظي من TradingView عبر tvDatafeed”””
tv = _get_tv()
if tv is None or ticker not in TV_SYMBOLS:
return None
sym, exch = TV_SYMBOLS[ticker]
try:
...
@st.cache_data(ttl=60, show_spinner=False)
def _yf_quote_fallback(ticker):
“”“يُستخدم فقط إذا فشل TradingView”””
...
ترتيب الأولوية:
1. Binance RT   ← كريبتو
2. TradingView  ← كل شيء
3. Yahoo        ← احتياطي أخير
“””
# كريبتو: Binance أسرع وأدق
...
return q
# Yahoo احتياطي
return _yf_quote_fallback(ticker)

@st.cache_data(ttl=120, show_spinner=False)
def _binance_ohlcv(sym, interval, limit):
“”“OHLCV من Binance — للكريبتو”””
...
@st.cache_data(ttl=120, show_spinner=False)
def _tv_ohlcv(ticker, interval, n_bars):
“”“OHLCV من TradingView عبر tvDatafeed”””
tv = _get_tv()
if tv is None or ticker not in TV_SYMBOLS:
return None
sym, exch = TV_SYMBOLS[ticker]
iv_attr = TV_INTERVAL.get(interval, “in_daily”)
try:
iv_obj = getattr(Interval, iv_attr, Interval.in_daily)
df = tv.get_hist(symbol=sym, exchange=exch,
interval=iv_obj, n_bars=n_bars)
if df is None or df.empty:
return None
df = df.dropna()
# tvDatafeed يُرجع أعمدة lowercase
df.columns = [c.capitalize() for c in df.columns]
# أحياناً يُرجع عمود “Symbol” — نحذفه
for drop_col in [“Symbol”, “symbol”]:
...
@st.cache_data(ttl=120, show_spinner=False)
def _yf_ohlcv_fallback(ticker, period, interval):
“”“Yahoo Finance — احتياطي أخير لـ OHLCV”””
if not _YF_AVAILABLE:
return None
try:
import yfinance as yf
...
“””
ترتيب الأولوية:
1. Binance OHLCV  ← كريبتو فقط
2. TradingView    ← كل الأصول
3. Yahoo          ← احتياطي أخير
“””
n_bars = TV_BARS.get(interval, 350)

```
# كريبتو: Binance
if ticker in CRYPTO_SYMBOLS:
    df = _binance_ohlcv(CRYPTO_SYMBOLS[ticker], interval,
                        {"15m":500,"1h":500,"4h":400,"1d":300,"1wk":200}.get(interval,300))
    if df is not None and len(df) >= 50:
        return df

# TradingView
df = _tv_ohlcv(ticker, interval, n_bars)
if df is not None and len(df) >= 50:
    return df

# Yahoo احتياطي
return _yf_ohlcv_fallback(ticker, period, interval)
```
